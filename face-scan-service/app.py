...
def _get_embedding_from_path(img_path: str) -> List[float]:
...
def search_faces(payload: SearchRequest) -> Dict[str, Any]:
    ...
    try:
        scan_embedding = _get_embedding_from_path(scan_path)
    except Exception as exc:
    ...
    for candidate in payload.candidates:
        ...
        try:
            if candidate.path in EMBEDDING_CACHE:
                candidate_embedding = EMBEDDING_CACHE[candidate.path]
            else:
                response = requests.get(candidate.url, timeout=REQUEST_TIMEOUT)
                response.raise_for_status()
                candidate_path = _write_temp_image_bytes(response.content)
                try:
                    candidate_embedding = _get_embedding_from_path(candidate_path)
                    EMBEDDING_CACHE[candidate.path] = candidate_embedding
                finally:
                    try:
            ...
            score = _cosine_similarity(scan_embedding, candidate_embedding)
            if score >= float(payload.threshold):
                matches.append(
                    {
                        "path": candidate.path,
                        "url": candidate.url,
